...
def test_writeconfig_uses_placeholder_values(tmp_path):
    ...
    assert parser.get(
        configuration.TIMEZONE_SECTION,
        configuration.TIMEZONE_OPTION) == configuration.DEFAULT_TIMEZONE
    assert parser.get(
        configuration.LANGUAGE_SECTION,
        configuration.LANGUAGE_OPTION) == configuration.DEFAULT_LANGUAGE
    assert parser.getfloat(configuration.REQUEST_SECTION,
                           configuration.REQUEST_TIMEOUT_KEY
...
def test_read_monitor_list_returns_structured_items(tmp_path, monkeypatch,
                                                    caplog):
...
def test_read_monitor_list_returns_empty_when_total_missing(
...
def test_read_monitor_list_creates_template_when_missing(
        tmp_path, monkeypatch, caplog):
    monkeypatch.setenv(configuration.LOG_DIR_ENV, str(tmp_path))
    ...
    relevant_records = [
        ...
        if record.name == configuration.LOGGER.name
    ]
    assert relevant_records
    assert not any(record.levelno >= logging.ERROR
                   for record in relevant_records)
    assert any("sample template" in record.getMessage().lower()
               for record in relevant_records)
...
def test_read_monitor_list_recovers_missing_template(tmp_path, monkeypatch,
                                                     caplog):
    ...
    assert any("template file missing" in record.getMessage().lower()
               for record in relevant_records)
...
def test_write_config_parser_uses_utf8(tmp_path, monkeypatch):
    ...
    parser.set("General", "name", "API Monitor")
    ...
    def strict_open(self, mode="r", *args, **kwargs):
        if self == target and "w" in mode:
            ...
            if kwargs.get("encoding") is None:
                raise UnicodeEncodeError("cp1252", b"", 0, 1,
                                         "encoding required")
    ...
    assert "API Monitor" in target.read_text(encoding="utf-8")
...
def test_set_preferences_updates_values(tmp_path, monkeypatch):
    ...
    assert (parser.get(configuration.LANGUAGE_SECTION,
                       configuration.LANGUAGE_OPTION) == "en_US")
    assert (parser.get(configuration.TIMEZONE_SECTION,
                       configuration.TIMEZONE_OPTION) == "-5")
    assert (parser.get(
        configuration.PREFERENCES_SECTION,
        configuration.THEME_DISPLAY_NAME_OPTION) == "Workspace Dark")
    assert (parser.get(configuration.PREFERENCES_SECTION,
                       configuration.THEME_DESCRIPTION_OPTION) ==
            "Dark theme designed for low-light work")
    assert (parser.get(configuration.PREFERENCES_SECTION,
                       configuration.THEME_HIGH_CONTRAST_OPTION) == "false")
...
def test_set_preferences_supports_chinese_values(tmp_path, monkeypatch):
    ...
    configuration.set_preferences({
        "theme": "workspace_dark",
        "theme_display_name": "Night theme",
        "theme_description": "Suitable for night use",
        "theme_high_contrast": False,
    ...
    assert "Night theme" in text
    assert "Suitable for night use" in text
...
def test_set_preferences_clears_theme(tmp_path, monkeypatch):
    ...
    assert not parser.has_option(configuration.PREFERENCES_SECTION,
                                 configuration.THEME_OPTION)
    assert not parser.has_option(configuration.PREFERENCES_SECTION,
                                 configuration.THEME_DISPLAY_NAME_OPTION)
    assert not parser.has_option(configuration.PREFERENCES_SECTION,
                                 configuration.THEME_DESCRIPTION_OPTION)
    assert not parser.has_option(configuration.PREFERENCES_SECTION,
                                 configuration.THEME_HIGH_CONTRAST_OPTION)
    prefs = configuration.get_preferences()
    assert prefs["theme"] is None
    assert prefs["theme_display_name"] is None
    assert prefs["theme_description"] is None
...
def test_read_mail_configuration_falls_back_to_root_config(
...
def test_write_monitor_list_supports_chinese_content(tmp_path, monkeypatch):
    ...
    assert "alert@example.com" in text
...
def test_configure_logging_installs_handlers(tmp_path, monkeypatch):
    ...
    matching_handlers = [
        handler for handler in root_logger.handlers
        if getattr(getattr(handler, "formatter", None), "_fmt", None) ==
        settings.fmt and not isinstance(handler, RotatingFileHandler)
    ]
...
def test_get_logging_preferences_returns_user_values(tmp_path, monkeypatch):
    ...
    parser.set("Logging", "log_directory", str(
        (tmp_path / "AltLogs").resolve()))
    ...
    assert Path(prefs["directory"]).resolve() == (tmp_path /
                                                  "AltLogs").resolve()
